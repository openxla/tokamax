...
def array_numeric_summary(x: jax.Array) -> NumericSummary:
  ...
  return NumericSummary(
      has_inf=bool(np.isinf(x_).any()),
      has_nan=bool(np.isnan(x_).any()),
      min=float(np.nanmin(x_)),
      max=float(np.nanmax(x_)),
      mean=float(np.nanmean(x_)),
      mean_abs=float(np.nanmean(np.abs(x_))),
  )
...
async def _standard_normal(rng, size, dtype) -> np.ndarray:
  ...
  try:
    ...
    dtype_ = jnp.float32
  out = np.empty(math.prod(size), dtype)
  ...
  if num_chunks <= 1:
    if out.size > 0:
      await asyncio.to_thread(generate_chunk, rng, 0, out.size)
  else:
    futs = []
    for i, rng in enumerate(rng.spawn(num_chunks)):
      start = i * _RNG_CHUNK_SIZE
      end = min(start + _RNG_CHUNK_SIZE, out.size)
      futs.append(asyncio.to_thread(generate_chunk, rng, start, end))
...
async def _random_initialize_async(x: PyTree, seed: int = 0) -> PyTree:
  ...
  async def init_with_layout(rng, x):
    if isinstance(x, ArrayInitializer):
      return jax.device_put(await asyncio.to_thread(x, rng))
    if isinstance(x, qwix.QArray):
      ...
      if abstract_qvalue and abstract_scale:
        ...
        return await asyncio.to_thread(quantize, values, x.qtype)
    ...
    if jnp.issubdtype(dtype, jnp.floating):
      y = await _standard_normal(rng, x.shape, dtype)
    elif dtype.name == 'bool':
      bernoulli = lambda: rng.binomial(n=1, p=0.5, size=x.shape).astype(dtype)
      y = await asyncio.to_thread(bernoulli)
    elif 'int' in dtype.name:
      y = await asyncio.to_thread(_int_initializer, rng, x.shape, dtype)
    else:
