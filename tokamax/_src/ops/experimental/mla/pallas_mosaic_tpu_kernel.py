...
def _mla_ragged_paged_attention_kernel(
  ...
  def batch_flash_attention(
    ...
    pv = jnp.einsum(
        "bnm,bnd->bnd" if not transpose_kv_cache else "bnm,bdm->bnd",
        p,
        kv_c,
        preferred_element_type=jnp.float32,
    )
  ...
  def _fetch_bkv(batch_start_seq_idx, bkv_idx, bkv_sem_idx, *, wait=False):
    ...
    for b in range(batch_size):
      sem = sems.at[0, b, bkv_sem_idx]
      # bkvc_x2_ref shape: [2, batch_size, bkv_sz_per_kv_packing + 2, kv_packing,
      # lkv_dim]
      bkvc_vmem_ref = bkvc_x2_ref.at[bkv_sem_idx, b]
      bkvpe_vmem_ref = bkpe_x2_ref.at[bkv_sem_idx, b]
  ...
  def _pack_new_transposed_kv(bkv_sem_idx, bkv_idx, offsets, update_szs):
    ...
    """
    debug_print(f"[RPA debug] -----------_pack_new_transposed_kv-----------")
    for b in range(batch_size):
      offset = offsets[b]
  ...
  def _pack_new_kv(bkv_sem_idx, offsets, update_szs):
    ...
    Args:
      ...
      update_szs: A list of the number of new tokens to be packed for a batch.
    """
    debug_print(f"[RPA debug] -----------_pack_new_kv-----------")
    for b in range(batch_size):
      offset = offsets[b]
      update_sz = update_szs[b]
  ...
  def _update_transposed_kv_cache(
    ...
    debug_print(
        f"[RPA debug] -----------_update_transposed_kv_cache-----------"
    )
    seq_idx = batch_start_seq_idx + b
...
def mla_ragged_paged_attention(
  ...
  """
  if (
      num_kv_pages_per_block is None
      or num_queries_per_block is None
      or vmem_limit_bytes is None
  ):
    raise ValueError(
        "num_kv_pages_per_block, num_queries_per_blocks, "
        "and vmem_limit_bytes must be specified."
    )
  ...
  def run_mla_kernel(
      ql_nope: jax.Array,  # [max_num_tokens, actual_num_q_heads,
      #  actual_lkv_dim]
      q_pe: jax.Array,  # [max_num_tokens, actual_num_q_heads, actual_r_dim]
      new_kv_c: jax.Array,  # [max_num_tokens, actual_lkv_dim]
      new_k_pe: jax.Array,  # [max_num_tokens, actual_r_dim]
    ...
    # We need two additional words because the beginning and end of the new KV
    # may both not be aligned to kv_packing boundaries.
    # Example:
    #
    # T0 T4     K2  K6
    # T1        K3
    ...
    scope_name = f"MLA-{case.symbol}-bq_{bq_sz}-bkvp_{bkv_p}-p_{page_size}-bsz_{batch_size}-vmem_{vmem_limit_bytes/1024/1024}"
