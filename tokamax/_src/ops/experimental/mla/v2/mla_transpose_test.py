...
from tokamax._src.ops.experimental.mla.v2.transpose import (
    prev_closest_valid_divisor,
    xpose_full,
    xpose_pipeline,
)
...
class TestXposePipelineTiling(parameterized.TestCase):
  ...
  def test_non_divisible_n_tile(self):
    ...
    input_data = jax.random.normal(
        jax.random.PRNGKey(0), shape, dtype=jnp.float8_e4m3fn
    )
    ...
    with patch(
        'tokamax._src.ops.experimental.mla.v2.transpose.logger'
    ) as mock_logger:
      result = xpose_pipeline(
          input_data, transpose_axes=(1, 0, 2), n_tile=160, m_tile=64
      )[0]
  ...
  def test_clamped_n_tile(self):
    ...
    with self.assertNoLogs(
        'tokamax._src.ops.experimental.mla.v2.transpose',
        level='WARNING',
    ):
      result = xpose_pipeline(
          input_data, transpose_axes=(1, 0, 2), n_tile=160, m_tile=64
  ...
  def test_vmem_limit_bytes_allows_large_tiles(self):
    ...
    shape = (384, 128, 512)
    input_data = jax.random.normal(
        jax.random.PRNGKey(0), shape, dtype=jnp.bfloat16
    )
    result = xpose_pipeline(
        input_data,
        transpose_axes=(1, 0, 2),
