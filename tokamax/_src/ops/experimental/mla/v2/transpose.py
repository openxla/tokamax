...
from tokamax._src.ops.experimental.mla.v2.kv_utils import get_dtype_packing
...
def xpose_pipeline(input: jax.Array,
                   *,
                   ...
                   parallel_axis: int = 0,
                   pipeline_axis: int = 1,
                   vmem_limit_bytes: int | None = None) -> list[jax.Array]:
  """Double buffer transpose custom call implementation.
  ...
  Args:
    ...
    transpose_axes: transpose ordering
    ...
    vmem_limit_bytes: the vmem limit for the pallas kernel. With the default
      ...
      (--xla_tpu_scoped_vmem_limit_kib, 32 MiB by default), which large tile
      shapes can exceed at compile time.
  """
  ...
  n_tile = (
      n_tile
      if n_tile <= input.shape[parallel_axis]
      else input.shape[parallel_axis]
  )
  m_tile = (
      m_tile
      if m_tile <= input.shape[pipeline_axis]
      else input.shape[pipeline_axis]
  )
  ...
  n_tile_new = prev_closest_valid_divisor(
      input.shape[parallel_axis], n_tile, multiple_of=sublane_multiple
  )
  if input.shape[parallel_axis] % n_tile_new != 0:
    raise ValueError(
        f"No divisor of parallel axis size {input.shape[parallel_axis]} is both"
        f" <= {n_tile} and divisible by {sublane_multiple}"
        f" (dtype={input.dtype}). Consider increasing n_tile and/or padding"
        " your input to be suble-aligned (i.e. a multiple of"
        f" {sublane_multiple})."
    )
  ...
  if input.shape[pipeline_axis] % m_tile_new != 0:
    raise ValueError(
        f"No divisor of pipeline axis size {input.shape[pipeline_axis]} is both"
        f" <= {m_tile} and divisible by {sublane_multiple}"
        f" (dtype={input.dtype}). Consider increasing n_tile and/or padding"
        " your input to be suble-aligned (i.e. a multiple of"
        f" {sublane_multiple})."
    )
  ...
  grid = (
      input.shape[parallel_axis] // n_tile,
  ...
  return pl.pallas_call(
      ...
      in_specs=input_specs,
      out_specs=output_specs,
      out_shape=[jax.ShapeDtypeStruct(shape=output_shape, dtype=input.dtype)],
      name=scope_name,
  )(input)
