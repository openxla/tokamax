...
from tokamax._src.ops.experimental.mla.v2 import kv_utils
from tokamax._src.ops.experimental.mla.v2.transpose import xpose_pipeline

logger = logging

_XPOSE_N_TILE_SIZE = int(os.getenv("MLA_XPOSE_N_TILE_SIZE", "160"))
...
def static_validate_inputs(
    ...
    *,
    ...
    num_kv_pages_per_blocks: Sequence[int] | None = None,
    num_queries_per_blocks: Sequence[int] | None = None,
...
def _mla_ragged_paged_attention_kernel(
    # Prefetch
    kv_lens_ref: Any,  # [max_num_seqs]
    page_indices_ref: Any,  # [max_num_seqs * pages_per_seq]
    cu_q_lens_ref: Any,  # [max_num_seqs + 1]
    start_end_seq_idx_ref: Any,  # [2] (start_seq_idx, end_seq_idx)
    sem_ids_ref: Any,  # [3] (bq_sem_idx, bkv_sem_idx, bo_sem_idx)
    bo_ids_ref: Any,  # [4] (bo_sem_0_seq_idx, bo_sem_1_seq_idx, bo_sem_0_bo_idx, bo_sem_1_bo_idx)
    bkv_update_ids_ref: Any,  # [batch_size, 6] (bkv_sem_0_seq_idx, bkv_sem_1_seq_idx, bkv_sem_0_offset, bkv_sem_1_offset, bkv_sem_0_sz, bkv_sem_1_sz) * batch_size
    # Input
    ql_nope_hbm_ref: Any,  # [max_num_tokens, num_q_heads, lkv_dim]
    q_pe_hbm_ref: Any,  # [max_num_tokens, num_q_heads, r_dim]
    new_kv_c_hbm_ref: Any,  # [max_num_tokens_per_kv_packing, kv_packing, lkv_dim] if not transpose_kv_cache else [lkv_dim, max_num_tokens]
    new_k_pe_hbm_ref: Any,  # [max_num_tokens_per_kv_packing, kv_packing, r_dim] if not transpose_kv_cache else [r_dim, max_num_tokens]
    cache_kv_hbm_ref: Any,  # [total_num_pages, page_size_per_kv_packing, kv_packing, align_to(lkv_dim + r_dim, 128)] if not transpose_kv_cache else [total_num_pages, align_to(lkv_dim + r_dim, 128), page_size]
    # Output
    o_hbm_ref: Any,  # [max_num_tokens, num_q_heads, lkv_dim]
    updated_cache_kv_hbm_ref: Any,  # [total_num_pages, page_size_per_kv_packing, kv_packing, align_to(lkv_dim + r_dim, 128)] if not transpose_kv_cache else [total_num_pages, align_to(lkv_dim + r_dim, 128), page_size]
    # Scratch
    bkvc_x2_ref: Any,  # [2, batch_size, bkv_buf_sz_per_kv_packing, kv_packing, lkv_dim] if not transpose_kv_cache else [2, batch_size, lkv_dim, bkv_sz + 256]
    bkpe_x2_ref: Any,  # [2, batch_size, bkv_buf_sz_per_kv_packing, kv_packing, r_dim] if not transpose_kv_cache else [2, batch_size, r_dim, bkv_sz + 256]
    bq_nope_x2_ref: Any,  # [2, batch_size, bq_sz, num_q_heads, lkv_dim]
    bq_rope_x2_ref: Any,  # [2, batch_size, bq_sz, num_q_heads, r_dim]
    bo_x2_ref: Any,  # [2, batch_size, bq_sz, num_q_heads, lkv_dim]
    sems: Any,  # [4, batch_size, 2]
    l_ref: Any,  # [batch_size, bq_sz * num_q_heads, 128],
    m_ref: Any,  # [batch_size, bq_sz * num_q_heads, 128],
    acc_ref: Any,  # [batch_size, bq_sz * num_q_heads, lkv_dim],
    *,
    static_q_len: int | None,
    sm_scale: float,
    mask_value: float,
    s_dtype: jnp.dtype | Any,
  ...
  def flash_attention_step1_qk_softmax(
    ...
    if bkv_sz % 256 == 0:
      ...
      q = jnp.concatenate([ql_nope, q_pe], axis=-1)
      k = jnp.concatenate(
          [kv_c, k_pe], axis=-1 if not transpose_kv_cache else -2
      )
      s = jnp.einsum(
          "nd,md->nm" if not transpose_kv_cache else "nd,dm->nm",
          q,
    ...
    if s.shape[0] % num_q_heads == 0 and s.shape[1] % 128 == 0:
      ...
      threshold = (
          kv_len - q_len + (bq_idx * bq_sz + bq_offset) - bkv_idx * bkv_sz
      )
      ...
      if not wait:
        ...
        for i in range(bkv_p):
          ...
          debug_print(
              ...
              " page_size_per_kv_packing={}, sz_per_kv_packing={}, page_idx={},"
              " page_indices_ref[page_idx]={}",
              bkv_p,
              i,
              page_size_per_kv_packing,
  ...
  def _pack_new_transposed_kv(bkv_sem_idx, bkv_idx, offsets, update_szs):
    ...
    Args:
      ...
      offsets: A list of the starting token offset in the KV cache where the new
        KVs begin for a batch.
    ...
    if not wait:
      ...
      def loop_body(i, states):
        ...
        debug_print(
            ...
            " words_to_transfer={}, curr_word_in_vmem={}, sz_words={},"
            " page_idx={}",
            i,
            curr_word_in_page,
            words_to_transfer,
  ...
  # TODO: Split out the KV handling functions into separate files.
  if not transpose_kv_cache:
  ...
  def process():
    ...
    def compute_with_bq(bq_idx, _):
      ...
      def compute_with_bkv(bkv_idx, _):
        ...
        # TODO: Split out the KV handling functions into separate
        # files.
...
def prepare_q_nope_inputs(
  ...
  The `q_nope` einsum emits in head-major layout (N, T, L). It needs to be
  transposed
...
def prepare_kv_inputs(kv: jax.Array, kv_packing: int) -> jax.Array:
...
def prepare_kv_inputs_for_transposed_kv_cache(kv: jax.Array) -> jax.Array:
  max_num_tokens, actual_head_dim = kv.shape
...
def prepare_outputs(
    out: jax.Array,  # [max_num_tokens, num_q_heads, head_dim]
...
def mla_ragged_paged_attention(
  ...
  Args:
    ...
    two_step_flash_attention: if true, kernel compute QK attention and AV
      attention in two separate steps to make them be more parallelized with
      VALU ops instead of batched matmul in one step.
  ...
  batch_distribution = (
      distribution[0] // decode_batch_size
  ) * decode_batch_size
  # Batched decode
...
def cdiv(a: int | jax.Array, b: int) -> int | jax.Array:
  return (a + b - 1) // b
...
def _ref_mqa_attention(
    q_i: jax.Array,
    k_i: jax.Array,
    v_i: jax.Array,
    q_span_offset: int | jax.Array,
    *,
    sm_scale: float = 1.0,
    sliding_window: int | None = None,
    soft_cap: float | None = None,
  ...
  """JIT-compiled attention computation for a single sequence."""
  q_len = q_i.shape[0]
  kv_len = k_i.shape[0]
  ...
  # Causal mask (broadcasted in 2D)
  q_span = (q_span_offset + jnp.arange(q_len, dtype=jnp.int32))[:, None]
  kv_span = jnp.arange(kv_len, dtype=jnp.int32)[None, :]
  mask = q_span < kv_span
  if sliding_window is not None:
    mask = jnp.logical_or(mask, q_span - sliding_window >= kv_span)
  if soft_cap is not None:
    attn = soft_cap * jnp.tanh(attn / soft_cap)
  attn = jnp.where(mask[None, :, :], mask_value, attn)
