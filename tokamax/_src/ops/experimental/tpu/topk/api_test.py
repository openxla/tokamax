...
class ApiTest(parameterized.TestCase):
  ...
  def test_topk_api_xla(self):
    ...
    @jax.jit
    def f(keys):
      return api.top_k(keys, k, implementation="xla")
    ...
    with self.subTest("correct_implementation_used"):
      opspecs = hlo_utils.get_opspecs(f.lower(keys), include_xla_kernels=False)
  ...
  def test_topk_api_mosaic_tpu(self):
    ...
    @jax.jit
    def f(keys):
      return api.top_k(keys, k, implementation="mosaic_tpu")
