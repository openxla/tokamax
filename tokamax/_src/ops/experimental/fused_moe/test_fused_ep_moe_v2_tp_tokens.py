...
from tokamax._src.ops.experimental.fused_moe import host
...
def _make_inputs(mesh, groups, rows, weight_format):
    ...
    for i, group in enumerate(groups):
        for device in group:
            owner[device] = i
    rank = jax.device_put(
        np.arange(ep, dtype=np.int32).reshape(ep, 1),
    ...
    def init(rank_l):
        ...
        tk = jax.random.split(
            jax.random.fold_in(jax.random.key(123), group_id), 2)
        ...
        def weights(key, width):
            raw = jax.random.normal(key, (2, 512, width)) / jnp.sqrt(512.)
...
def _run_layer(mesh,
    ...
    if reference_padding:
        ...
        def pad(x, gate):
            return (jnp.pad(x, ((0, reference_padding), (0, 0))),
                    jnp.pad(gate, ((0, reference_padding), (0, 0)),
                            constant_values=-jnp.inf))

        x, gate = jax.shard_map(pad,
                                mesh=mesh,
                                in_specs=(P(AXIS), P(AXIS)),
                                out_specs=(P(AXIS), P(AXIS)),
                                check_vma=False)(inputs[0], inputs[-1])
    ...
    if reference_padding:
        result = jax.shard_map(lambda rows: rows[:input_rows],
                               mesh=mesh,
                               in_specs=P(AXIS),
                               out_specs=P(AXIS),
                               check_vma=False)(result)
...
def _assert_outputs_match(actual, reference, rows, groups):
    ...
    for shard in actual.addressable_shards:
        ...
        assert np.linalg.norm(
            want) > 1e-8, "zero output is not a useful oracle"
        delta = got - want
        relative = float(
            np.linalg.norm(delta) / max(np.linalg.norm(want), 1e-20))
        worst = float(
            np.max(
...
@pytest.mark.parametrize("weight_format", [WeightFormat.FP8, WeightFormat.FP4],
                         ids=["fp8", "fp4"])
@pytest.mark.parametrize("sharded_plan", [False, True],
                         ids=["full", "sharded"])
@pytest.mark.parametrize("groups,rows", [
...
def test_single_host_replicas_match_original_layer(single_host_mesh, groups,
                                                   rows, weight_format,
                                                   sharded_plan):
...
@pytest.mark.parametrize("sharded_plan", [False, True],
                         ids=["full", "sharded"])
@pytest.mark.parametrize("scoring_fn,renormalize,scale", [
...
def test_single_host_replica_routing_options(single_host_mesh, weight_format,
                                             sharded_plan, scoring_fn,
                                             renormalize, scale):
    mesh = single_host_mesh
    ...
    unit_scale = _run_layer(mesh, rank, inputs, groups, weight_format,
                            sharded_plan, **options)
    ...
    np.testing.assert_allclose(np.asarray(actual).astype(np.float32),
                               np.asarray(unit_scale).astype(np.float32) *
                               scale,
                               rtol=0.008,
                               atol=1e-7)
    without_bias = _run_layer(mesh,
                              rank,
                              inputs,
                              groups,
                              weight_format,
...
def test_fp4_replicas_match_original_layer(case, rows):
    ...
    if case == 'tp1':
        ...
        groups = tuple(
            tuple(i + h for i in g) for h in (0, 8) for g in _TP2_PERMUTED)
    elif case == 'tp2_cross_host':
        groups = tuple((i + 8, i) for i in range(8))
    else:
        groups = tuple(
            tuple(i + h for i in g) for h in (0, 8) for g in _TP4_PERMUTED)
