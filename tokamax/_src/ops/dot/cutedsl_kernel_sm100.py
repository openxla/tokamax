...
from tokamax._src.ops.dot import cutedsl_common as common
...
def dot_kernel(
  ...
  if k_lhs != k_rhs:
    raise ValueError(
        f"Contraction dim mismatch: {lhs.shape[1]=}, {rhs.shape[1]=}."
    )

  if (lhs.dtype, rhs.dtype, out_dtype) != 3 * (jnp.float16,):
    raise ValueError(
        "Only float16 x float16 -> float16 is supported, got:"
        f" {lhs.dtype=} {rhs.dtype=} {out_dtype=}."
    )
  ...
  acc_stage = config.num_acc_stages
