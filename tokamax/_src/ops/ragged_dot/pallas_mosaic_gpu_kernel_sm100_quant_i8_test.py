...
_CONFIG = pallas_mosaic_gpu.Config(
    ...
    persistent=True,
    collective=False,
    use_native_int8_mma=True,
    grid_minor_dim=pallas_mosaic_gpu.common.MatmulDimension.M,
    grid_tile_width=1,
)


class PallasMosaicGpuKernelSm100QuantI8Test(test_base.RaggedDotTestBase):
  ...
  def test_wi4_abf16_quantized(
      self, tile_shape, use_as_qarray, activation, task
  ):
    self._test_quantized(
        None,  # a_dtype
        "int4",
