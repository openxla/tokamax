...
def _bwd(
  ...
  def kernel_dq(q_ref, k_ref, v_ref, dout_ref, m_ref, l_ref, delta_ref, dq_ref,
    ...
    def kv_pipeline(
      ...
      p = jnp.exp(s - broadcast(m)) / broadcast(l)
  ...
  def kernel_dkv(q_ref, k_ref, v_ref, dout_ref, m_ref, l_ref, delta_ref,
    ...
    def q_pipeline(
      ...
      pT = jnp.exp(sT - broadcast(m)) / broadcast(l)
  ...
  dk, dv = plgpu.kernel(
      ...
      grid_names=("batch", "num_kv_tiles", "heads"),
      num_threads=compute_wgs + 1,
      thread_name="wg",
  )(q, k, v, dout, m, l, delta)
...
class PallasMosaicGpuFlashAttentionVjp(
  ...
  def _fwd(
    ...
    del precision, dropout_rate

    if not normalize_output:
      raise NotImplementedError("`normalize_output=False` not supported.")

    if self.use_base2:
      raise ValueError("use_base2 is not supported.")
    ...
    f = functools.partial(_bwd, logits_scale=logits_scale, config=config)
