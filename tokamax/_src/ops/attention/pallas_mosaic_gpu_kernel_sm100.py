...
def flash_attention_kernel(
  ...
  _ = """
  The kernel is split into 2 workgroups:
  - MMA: responsible for computing Q@K and P@V.
    - pv_mma_warp: responsible for computing P@V in MMA.
    - qk_mma_warp: responsible for computing Q@K in MMA.
    - pv_memory_warp: responsible for loading v from GMEM to SMEM.
    - qk_memory_warp: responsible for loading k from GMEM to SMEM.
  - Softmax: responsible for computing softmax in TMEM.
  """
  if bias is not None:
  ...
  def pre_reduce_k_range_per_qtile(range_ref):
    if range_ref is None:
      return None
    range_block = min(tile_q, range_ref.shape[-1])
    k_range_ = shape_lib.einshape("...(bq)->...bq", q=range_block)(range_ref)
    return (jnp.min(k_range_, -1), jnp.max(k_range_, -1))
  ...
  residuals = tuple(res for res in residuals)
