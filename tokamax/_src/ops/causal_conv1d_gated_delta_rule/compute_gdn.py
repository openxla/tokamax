...
def chunked_gdn_per_seq(
  ...
  # [num_v_heads, chunk, 1]
  g_cum_sum_log = fused_transpose_broadcast(
      g_cum_sum_log, src_dim=2, dst_dim=0
  )
...
def recurrent_gdn(
  ...
  if cfg.chunk_size == 1 and cfg.window_size == 1:
    # Reformulate the decode step into a single batched contraction via
    # jax.lax.dot_general over [B, n_v, 2, d_k] (CL 971045168).
    # Shapes:
    ...
    # Match [B, window_size=1, n_v, d_k, d_v]:
    new_recurrent_state = jnp.expand_dims(new_recurrent_state, axis=1)
