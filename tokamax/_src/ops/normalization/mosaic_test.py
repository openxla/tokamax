...
class PallasMosaicGpuNormalizationTest(parameterized.TestCase):
  ...
  def test_bench(self, kwargs):
    self._run_bench_spec(kwargs)

  @parameterized.named_parameters(test_base.NAMED_ARG_SPECS.items())
  def test_bench_input_output_alias(self, kwargs):
    # `x` is written over in place, so the output has to come out the same.
    self._norm_fn = mosaic.PallasMosaicGpuNormalization(input_output_alias=True)
    self._run_bench_spec(kwargs)

  def _run_bench_spec(self, kwargs):
