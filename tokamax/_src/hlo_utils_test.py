...
class DumpHloLibTest(parameterized.TestCase):
  ...
  def test_simple_pallas_triton(
      self,
  ):
    dtype = jnp.int32
    x = jnp.arange(8, dtype=dtype)
    lowered = add_vectors_pallas_triton.lower(x=x, y=x)
    kernel_info = hlo_utils.get_gpu_kernel_info_from_lowered(lowered)
  ...
  def test_pallas_norm(
    ...
    triton_kernels = hlo_utils.get_gpu_kernel_info_from_lowered(f_grad_lowered)

    self.assertLen(triton_kernels, 2)
    forward, vjp = triton_kernels
    ...
    # TODO(sbodenstein): add tests for axis once this is in the Pallas HLO.
  ...
  def test_gated_linear_unit_mosaic_gpu(
    ...
    kernels = hlo_utils.get_gpu_kernel_info_from_lowered(swiglu.lower(x, w))
  ...
  def test_jax_triton_simple(self):
    ...
    kernels = hlo_utils.get_gpu_kernel_info_from_lowered(
        add_jax_triton.lower(x, y)
    )
    self.assertLen(kernels, 1)
    kernels = kernels[0]
    self.assertEqual(kernels.kernel_name, 'add_kernel')
    self.assertEqual(kernels.type, 'triton')
  ...
  def test_get_opspecs_from_lowered_jax(self):
    # TODO(stzeng): Find a better place to put the kernel spec rather than
    ...
    op_specs = hlo_utils.get_opspecs(norm_lowered)
    self.assertLen(op_specs, 2)
    self.assertIsInstance(op_specs[0].op, pl_norm.PallasTritonNormalization)
  ...
  def test_opspecs_round_trip(self):
    # TODO(stzeng): Add a test for vmap.
    ...
    op_specs = hlo_utils.get_opspecs(normalization_fn_lowered)
    ...
    op_specs = hlo_utils.get_opspecs(test_ragged_dot_lowered)
    self.assertLen(op_specs, 1)
